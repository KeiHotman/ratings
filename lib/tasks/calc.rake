...
namespace :calc do
  ...
  task similarities: :environment do
    ...
    subject_users.each do |subject_user|
      target_users.each do |target_user|
        ...
        subject_rating_average = subject_user.ratings.where(item: both_rated_items, taken: true, prediction: false).average(:score)
        target_rating_average  = target_user.ratings.where(item: both_rated_items, taken: true, prediction: false).average(:score)
  ...
  task predictions: :environment do
    ...
    users.each do |user|
      # "類似ユーザ"を取得
      similar_users = User.joins(:targeted_similarities).merge(UsersSimilarity.where(user: user).where("value > 0.4").order('value desc').limit(2))

      # 全アイテムに対する予測評価値の算出
      items.each do |item|
        # 対象ユーザの対象アイテムにおける評価値を検索、なければ生成
        rating = Rating.find_or_initialize_by(user: user, item: item)
        next if (!rating.prediction && rating.persisted? && rating.taken) # 評価値登録済みの場合は省略
        ...
        similar_users.each do |similar_user|
          ...
          similars_rating = Rating.find_by(user: similar_user, item: item, prediction: false, taken: true)

          # "対象ユーザと類似ユーザ間の類似度"が存在する時
          if similars_rating
            # "対象ユーザと類似ユーザ間の類似度"と"類似ユーザの対象アイテムの評価値偏差"の積を"類似ユーザ評価値偏差・類似度積和"に加える
            similarity_deviation_products_sum += similarity.value * (similars_rating.score - similar_user.taken_ratings.average(:score))
        ...
        begin
          if total_similarity != 0
            ...
            prediction = user.taken_ratings.average(:score) + similarity_deviation_products_sum / total_similarity
            rating.assign_attributes(score: prediction, prediction: true, taken: false)
            rating.save
          end
        rescue => e
          puts "Error: user: #{user.name}, item: #{item.name}, #{e.message}"
        end
      end
    end
